...
def assert_gate_failure(
    report: dict, gate: str, mechanism: str | None, exit_code: int, returncode: int
):
...
class TestCleanCopyPasses:
    ...
    def test_report_carries_numeric_duration_per_gate(self, tmp_path):
        # CI-SPEED-PLAN M3c: every gate entry carries the wall-clock
        # seconds it consumed, so cost wins and regressions are visible
        # in the uploaded report artifact without re-timing CI by hand.
        repo = make_repo(tmp_path / "durations")
        code, report = run_full(repo)
        assert code == 0, json.dumps(report, indent=2)
        assert report["gates"], "no gates in the report"
        for name, entry in report["gates"].items():
            duration = entry.get("duration")
            assert isinstance(duration, (int, float)), f"gate {name}: {entry}"
            assert not isinstance(duration, bool), f"gate {name}: {entry}"
            assert duration >= 0, f"gate {name}: {entry}"
...
class TestRuffDiffScoped:
    def test_formatting_violation(self, tmp_path):
        ...
        code, report = run_full(repo)
        assert_gate_failure(report, "ruff", "diff-scoped", 1, code)
...
class TestSuppressionScan:
    def test_type_ignore_on_added_line(self, tmp_path):
        ...
        assert entry, report["gates"]
        assert entry["status"] == "fail", report["gates"]
